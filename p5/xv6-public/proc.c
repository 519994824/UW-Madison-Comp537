...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  for (i = 0; i < MAX_WMAPS; i++) {
    if (curproc->wmap_regions[i].length > 0) {
        // Copy mapping info
        np->wmap_regions[i] = curproc->wmap_regions[i];

        // Handle file-backed mappings
        if (curproc->wmap_regions[i].f) {
            np->wmap_regions[i].f = filedup(curproc->wmap_regions[i].f);
        }
    }
  }
...
exit(void)
{
  ...
  for (int i = 0; i < MAX_WMAPS; i++) {
    if (curproc->wmap_regions[i].length > 0) {
      do_wunmap(curproc->wmap_regions[i].addr);
    }
  }
